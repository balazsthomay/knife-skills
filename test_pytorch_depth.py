#!/usr/bin/env python3
"""
Direct PyTorch vs HuggingFace visual comparison demo.
Real-time depth map visualization with performance comparison.
"""
...
def main():
    """Visual depth estimation comparison demo"""
    print("🚀 PyTorch vs HuggingFace Visual Comparison Demo")
    print("Controls:")
    print("  ESC - Exit")
    print("  SPACE - Pause/Resume")
    print("  S - Save screenshot + depth map")
    print("  M - Switch between PyTorch/HuggingFace")
    ...
    depth_estimators = {}
    current_model = "pytorch"  # Start with PyTorch
    ...
    print(f"🚀 Starting visual comparison with {configs[current_model]['name']}...")
    ...
    while True:
        ...
        # Display frame
        cv2.imshow('PyTorch vs HuggingFace Depth Comparison', combined_frame)
        ...
        if key == 27:  # ESC
            ...
            # Save RGB frame
            rgb_filename = f"pytorch_comparison_rgb_{timestamp}.jpg"
            cv2.imwrite(rgb_filename, frame)
            # Save depth visualization
            depth_filename = f"pytorch_comparison_depth_{timestamp}.jpg"
    ...
    for model_type, estimator in depth_estimators.items():
        ...
        print(f"  Estimated max FPS: {1000/final_stats['avg_inference_time_ms']:.1f}")
    ...
    print("\n👋 Visual depth comparison complete!")
